...
const excluirLivro = async (req, res) => {
  ...
  try {
    ...
    const { rows: emprestimos } = await conexao.query(
      "SELECT livro_id FROM emprestimos where livro_id = $1",
      [id]
    );
    ...
    let emprestimosLivro = 0;
    for (const emprestimo of emprestimos) {
      emprestimosLivro++;
    }

    if (emprestimosLivro > 0) {
      return res
        .status(400)
        .json(
          `Há ${emprestimosLivro} empréstimo(s) associado(s) à este livro. Por favor faça a exclusão antes de solicitar a remoção.`
        );
    }
